...
namespace eval ::rss-synd {
	...
	set rss(usgs) {
		"url"			"https://earthquake.usgs.gov/earthquakes/shakemap/rss.xml"
		"channels"		"#mpeg"
		"output"		"\[\00313USGS\003\]\@@item!description@@\ 4[Selengkapnya12: 14\@@item!guid@@\4]"
		"database"		"./scripts/det.db"
		"trigger"		"!news"
		"evaluate-tcl"	1
	}
...
proc ::rss-synd::init {args} {
	...
	set version(number)	"PKINACTION"
	set version(date)	"10-11-2009"
	...
	putlog "\002B_A_Y_O all news v$version(number)\002 ($version(date)): LoadeD..."
}
	...
	foreach name [array names rss] {
		...
		if {[string compare -nocase $text $feed(trigger)] == 0} {
			if {([[namespace current]::check_channel $feed(channels) $chan]) || ([string length $chan] == 0)} {
				...
				} else { putlog "\002RSS Warning\002: $error." }
			}
		}
	}
}
...
proc ::rss-synd::xml_list_create {xml_data} {
	...
	while {[string compare [set tag_start [[namespace current]::xml_get_position $xml_data $ptr]] ""]} {
		...
		if {[regexp -nocase -- {^!(\[CDATA|--)} $tag_string]} {
			...
			if {(![regexp {(\?|!|/)(\s*)$} $tag_args]) || \
				...
				regexp -indices -start $ptr -- "</$tag_name>" $xml_data tag_end
				set tag_end_first [lindex $tag_end 0]
				set tag_end_last [lindex $tag_end 1]

				# find out if we have any open tags of the same name inbetween
				set tmp_num [regexp -all -- "<$tag_name\(|.\[^>\]+\)>" [string range $xml_data $ptr $tag_end_last]]
...
#  regexp -indices -start $ptr {<(!\[CDATA\[.+?\]\]|!--.+?--|.+?)>} $xml_data -> tag_start
...
proc ::rss-synd::feed_output {feedlist data {odata ""}} {
	...
	foreach msg $msgs {
		# chan is a nick if run from a private trigger
		foreach chan $feed(channels) {
			if {([catch {botonchan $chan}] == 0) || ([[namespace current]::is_chan $chan] == 0)} {
...
proc ::rss-synd::feed_compare {feedlist odata data} {
	...
	if {[string compare -nocase [lindex $feed(tag-feed) 1] "feed"] == 0} {
		set cmp_items [list {0 "pubDate"} "children" "" 3 {0 "guid"} "children" "" 2 {0 "link"} "children" "" 1 {0 "title"} "children" "" 1 {0 "description"} "children" "" 0]
	} else {
		set cmp_items [list {0 "pubDate"} "children" "" 3 {0 "guid"} "children" "" 2 {0 "link"} "children" "" 1 {0 "title"} "children" "" 1 {0 "description"} "children" "" 0]
	}
